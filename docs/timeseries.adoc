...
== Active Information
Active information (AI) was introduced in <<Lizier2012>> to quantify information
storage in distributed computation. Active information is defined in terms of a
temporally local variant
[latexmath]
++++
a_{X,i}(k,b) = \log_b \frac{p(x_i^{(k)}, x_{i+1})}{p(x_i^{(k)})p(x_{i+1})}
++++
where the probabilities are constructed empirically from the entire time series.
From the local variant, the temporally global active information as
[latexmath]
++++
A_X(k,b) = \langle a_{X_i}(k,b) \rangle_i = \sum_{x_i^{(k)}, x_{i+1}} p(x_i^{(k)}, x_{i+1}) \log_b \frac{p(x_i^{(k)}, x_{i+1})}{p(x_i^{(k)})p(x_{i+1})}
++++
...
(https://github.com/elife-asu/inform/issues/24[yet!]).
...
[bibliography]
- [[[Lizier2012]]] J.T. Lizier, M. Prokopenko and A.Y. Zomaya,
  http://dx.doi.org/10.1016/j.ins.2012.04.016["Local measures of information
